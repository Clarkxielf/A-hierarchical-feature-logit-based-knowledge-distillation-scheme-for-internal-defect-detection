...
best_accuracy = 0.
save_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'student2.pkl')

writer = SummaryWriter(comment='Network1')
Loss = []
Acc_rate =[]
...
for epoch in range(EPOCH):
    ...
    for step, (b_x, b_y) in enumerate(train_loader):
        ...
        loss = loss_task(logits_s, b_y.to(device))
        loss_sum += loss
...
sio.savemat('loss_student.mat',{'loss':Loss_data})
sio.savemat('accuracy_student.mat',{'accuracy':Acc_data})
writer.close()
